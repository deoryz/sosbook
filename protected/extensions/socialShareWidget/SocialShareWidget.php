...
class SocialShareWidget extends CWidget
{
	...
	public function run()
	{
		...
		echo CHtml::tag('span', $opts, null, false);
		foreach ($this->services as $service)
		{
	        ...
	        if ($this->url == '')
		    {
		        $this->url = Yii::app()->request->hostInfo . Yii::app()->request->baseUrl . '/' . Yii::app()->request->pathInfo;
		    }
			$url = $this->serviceUrls[$service] . urlencode($this->url);
			echo CHtml::link('<i class="icon-'.$service.'" style="margin-top: -8px;"></i>', $url, array(
				'class' => ' social-provider-icon ' . $service, 
				'title' => $serviceName,
				'target' => '_blank',
			));
		}
